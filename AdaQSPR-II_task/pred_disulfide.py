...
def trans_epvalue2fh(epoxy_value_feat, smiles, func_group_feat, hydroxyl_feat):
    ...
    for i in range(len(epoxy_value_feat)):
        epoxy_value = epoxy_value_feat.iloc[i] if hasattr(epoxy_value_feat, 'iloc') else epoxy_value_feat[i]
        smi = smiles.iloc[i] if hasattr(smiles, 'iloc') else smiles[i]

        if epoxy_value == 0 and smi == 'C':
            real_hydroxyl_list.append(0)
            real_epoxy_list.append(0)
            continue

        elif (epoxy_value == 0 and smi != 'C'):
            ...
            if initial_epoxy == 0:
                ...
                raise ValueError(f"Row {i} data inconsistent: epoxy_value_feat={epoxy_value}, smiles='{smi}'")
            ...
            while ideal_epoxy_value > epoxy_value:
                n += 1
            ...
            if n == 0:
                ...
                A = epoxy_n - epoxy_n1 - (epoxy_value / 100) * (mw_n - mw_n1)
                B = epoxy_n1 - (epoxy_value / 100) * mw_n1
...
def prepare_data (Data):
    print("prediction")
    dataset_filename = "./pred_disulfide2/disulfide_ep2.xlsx"    ###change the file name
    property_map_mal = {
        '1': 'Tg_C',
        '2': 'Td5%',
        '3': 'TS',
...
def main(custom_data, cate3):
    ...
    print(p_inverse.max())
    dff = pd.read_excel('./pred_disulfide2/disulfide_ep2.xlsx', sheet_name='Sheet1') #### change the filename
    dff[f'{cate3}_pred'] = np.array(list(p_inverse))

    excel_file = f'./pred_disulfide2/pred/prediction_{cate3}_disulfide.xlsx'
