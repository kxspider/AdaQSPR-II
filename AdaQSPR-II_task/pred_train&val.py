...
#
def convert_epoxy_value_to_hydroxyl_epoxy(epoxy_value_feat, smiles, func_group_feat, hydroxyl_feat):
    """
    ...
    for i in range(len(epoxy_value_feat)):
        #
        epoxy_val = epoxy_value_feat.iloc[i] if hasattr(epoxy_value_feat, 'iloc') else epoxy_value_feat[i]

        #
        if epoxy_val == 0 and smi == 'C':
            real_hydroxyl_list.append(0)
            real_epoxy_list.append(0)
            continue

        #
        elif (epoxy_val == 0 and smi != 'C'):
            ...
            if initial_epoxy == 0:
                ...
                raise ValueError(f"Row {i} data inconsistent: epoxy_value_feat={epoxy_val}, smiles='{smi}'")
            ...
            while ideal_epoxy_value > epoxy_val:
                n += 1
            ...
            if n == 0:
                ...
                #  (epoxy_n*X + epoxy_n1*(1-X)) / (mw_n*X + mw_n1*(1-X)) = epoxy_val/100
                # A*X + B = 0
                A = epoxy_n - epoxy_n1 - (epoxy_val / 100) * (mw_n - mw_n1)
                B = epoxy_n1 - (epoxy_val / 100) * mw_n1
            ...
            if epoxy_error > 0.06 or hydroxyl_error > 0.06:
                count_exceed += 1
                print(f"Row {i + 2} data inconsistent", epoxy_error, real_epoxy_val, func_group_val, hydroxyl_error,
                      real_hydroxyl_val, hydroxyl_val, smi)
                print(f"Row {i + 2} correct data", real_epoxy_val, real_hydroxyl_val)  ###excel row number
...
def read_data_EP(filename, data_col_names, Data):
    ...
    epoxy_value_col_names = [d for d in df.keys() if 'epoxy value' in d]
    for col in epoxy_value_col_names:
        df[col].fillna(0, inplace=True)
    ...
    for idx, dcol in enumerate(smiles_col_names):
        ...
        if len(smiles) != len(df[tcol]):
            raise RuntimeError("SMILES column and target column have inconsistent lengths, please check the data logic")

        if idx < 3:
            epoxy_value_feat = df[epoxy_value_col_names[idx]]
            nn = convert_epoxy_value_to_hydroxyl_epoxy(epoxy_value_feat, smiles, func_group_feat, hydroxyl_feat)
            nnn = nnn + nn
            print(nnn)
            if nnn > 10:
                raise ValueError("Epoxy value calculation for hydroxyl and epoxy group counts is incorrect")
...
def prepare_data(Data):
    ...
    dataset_filename = "./pred_train_val/Ebnew.xlsx"  ###change the file name
    property_map_mal = {
        '1': 'Tg_C',
        '2': 'Td5%',
        '3': 'TS',
    ...
    cate3 = ""
    while True:
        train_property_input = input(
            "Training property (1:Glass transition temperature Tg, 2:5%wt thermal decomposition temperature Td5%, 3:Tensile strength TS, 4:epsilon e, 5:tan delta, 6:lectrical breakdown strength Eb: ")
...
def main(custom_data, cate3):
    ...
    print(p_inverse.max())
    dff = pd.read_excel('./pred_train_val/Ebnew.xlsx', sheet_name='Sheet1')  #### change the filename
    dff[f'{cate3}_pred'] = np.array(list(p_inverse))

    excel_file = f'./pred_train_val/pred/prediction_{cate3}_new.xlsx'
