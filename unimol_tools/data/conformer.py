...
def inner_smi2coords(smi, seed=42, mode='heavy', remove_hs=True, return_mol=False): #改fast为heavy remove_hs=True可改为False
    '''
    ...
    try:
        ...
        ## for fast test... ignore this ###
            ...
            AllChem.EmbedMolecule(mol, maxAttempts=800, randomSeed=seed) #maxAttempts=500原
            try:
                # some conformer can not use MMFF optimize
                AllChem.MMFFOptimizeMolecule(mol)
                coordinates = mol.GetConformer().GetPositions().astype(np.float32)
...
class UniMolV2Feature(object):
    ...
    def _init_features(self, **params):
        ...
        """
        self.seed = params.get('seed', 42)
        self.max_atoms = params.get('max_atoms', 128) #128有点小，改256 暂时未改
        self.data_type = params.get('data_type', 'molecule')
        self.method = params.get('method', 'rdkit_random')
        self.mode = params.get('mode', 'heavy') #改fast为heavy
        self.remove_hs = params.get('remove_hs', False) #改true为false
        if os.name == 'posix':
            self.multi_process = params.get('multi_process', True)
        else:
            self.multi_process = params.get('multi_process', False)
    ...
    def transform_raw(self, atoms_list, coordinates_list, smiles_list): #修改为三参数
        inputs = []
        for atoms, coordinates, smiles in zip(atoms_list, coordinates_list, smiles_list):
            mol = create_mol_from_atoms_and_coords(atoms, coordinates, smiles)
...
def create_mol_from_atoms_and_coords(atoms, coordinates, smiles): #修改为三参数
    """
    ...
    mol = Chem.RWMol(mol)
    num_atoms = mol.GetNumAtoms()  # non-hydrogen atom count
    new_conf = Chem.Conformer(num_atoms)
    if num_atoms != len(coordinates):
        raise ValueError("Error: The molecular coordinate format is incorrect")
    #print(coordinates)
    new_coords = np.array(coordinates, dtype=np.float64)
    new_conf.SetPositions(new_coords)
    mol.AddConformer(new_conf)
    Chem.SanitizeMol(mol)
    aaa = mol.GetConformer()
    bbb = aaa.GetPositions()
    #print(bbb)
    return mol
