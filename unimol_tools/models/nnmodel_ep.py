...
from .unimolv2_ep import UniMolV2Model
...
class NNModel_ep(object):
    ...
    def __init__(self, data, trainer, **params):
        ...
        # Features
        self.features = [d['unimol_input'] for d in data]
        self.data_feat = [d['feat'] for d in data] #
        self.data_smile_m = [d['smile_mark'] for d in data] #smiles
    ...
    def run(self):
        ...
        X = np.asarray(self.features).T #
        data_np = np.asarray(self.data_feat)
        Xfeat = np.asarray(self.data_feat).transpose(1, 0, 2)
        Xsmiles = np.asarray(self.data_smile_m).T   #

        y = np.asarray(self.data1['target'])    #
        group = (
            np.asarray(self.data1['group']) if self.data1['group'] is not None else None
        )
        ...
        for fold, (tr_idx, te_idx) in enumerate(self.data1['split_nfolds']):     #
            X_train, Xf_train, Xs_train, y_train = X[tr_idx], Xfeat[tr_idx], Xsmiles[tr_idx], y[tr_idx] #
            X_valid, Xf_valid, Xs_valid, y_valid = X[te_idx], Xfeat[te_idx], Xsmiles[te_idx], y[te_idx] #

            traindataset = MyDataset(X_train, Xf_train, Xs_train, y_train)  #
            validdataset = MyDataset(X_valid, Xf_valid, Xs_valid, y_valid)  #
            if fold > 0:
            ...
            if self.model_params.get('load_model_dir', None) is not None:
                load_model_path = os.path.join(
                    self.model_params['load_model_dir'], f'model_best.pth'
                )
        ...
        self.cv['metric'] = self.metrics.cal_metric(
            self.data1['target_scaler'].inverse_transform(y),
            self.data1['target_scaler'].inverse_transform(self.cv['pred']),
        )
    ...
    def evaluate(self, trainer=None, checkpoints_path=None):
        ...
        Xfeat = np.asarray(self.data_feat).transpose(1, 0, 2)
        Xsmiles = np.asarray(self.data_smile_m).T # 
        testdataset = MyDataset(X, Xfeat, Xsmiles, np.asarray(self.data1['target']))

        for fold in range(self.data1['kfold']):
...
class MyDataset(TorchDataset):  #
    def __init__(self, data, feat, mark, label=None):
        self.data = data
        self.feat = feat
        self.mark = mark
        self.label = label if label is not None else np.zeros((len(data), 1))
    ...
    def __getitem__(self, idx):
        return self.data[idx], self.feat[idx], self.mark[idx], self.label[idx]
