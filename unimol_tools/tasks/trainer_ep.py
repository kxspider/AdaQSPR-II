...
class Trainer_ep(object):
    ...
    def fit_predict_wo_ddp(
        ...
        train_dataloader = NNDataLoader(
            ...
            shuffle=True,
            collate_fn=model.batch_collate_fn2, #
            distributed=False,
            #drop_last=True,    #
        )
    ...
    def fit_predict_with_ddp(
        ...
        train_dataloader = NNDataLoader(
            ...
            batch_size=self.batch_size,
            shuffle=False,
            collate_fn=model.module.batch_collate_fn2,
            distributed=True,
            #drop_last=True,
        )
        optimizer, scheduler = self._initialize_optimizer_scheduler(
    ...
    def predict(
        ...
        if isinstance(model, DistributedDataParallel):
            batch_collate_fn = model.module.batch_collate_fn2 #
        else:
            batch_collate_fn = model.batch_collate_fn2 #
        dataloader = NNDataLoader(
            feature_name=feature_name,
            dataset=dataset,
            batch_size=self.batch_size,
            shuffle=False,
            collate_fn=batch_collate_fn, #2
    ...
    def inference_with_ddp(
        ...
        dataloader = NNDataLoader(
            ...
            batch_size=self.batch_size,
            shuffle=False,
            collate_fn=model.module.batch_collate_fn2,  #2
            distributed=True,
        )
        model = model.eval()
        repr_dict = {
