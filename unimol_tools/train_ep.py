...
class MolTrain_ep(object):
    ...
    def fit(self, data):
        ...
        .. code-block:: python
            ...
            clf.fit(custom_data)
        """
        self.datahub_list = [DataHub_ep(data=d, is_train=True, save_path=self.save_path, **self.config) for d in data] #修改适应字典的列表
        self.data = [dh.data for dh in self.datahub_list] #修改

        self.update_and_save_config()
        self.trainer = Trainer_ep(save_path=self.save_path, **self.config)  #
