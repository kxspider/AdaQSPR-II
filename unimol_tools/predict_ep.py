...
from .data import DataHub_ep
from .models import NNModel_ep
from .tasks import Trainer_ep
from .utils import YamlHandler, logger


class MolPredict_ep(object):
    ...
    def predict(self, data, save_path=None, metrics='none'):
        ...
        ## load test data
        self.datahub_list = [DataHub_ep(data=d, is_train=False, save_path=self.save_path, **self.config) for d in data]
        self.data = [dh.data for dh in self.datahub_list]

        self.config.use_ddp = False
        self.trainer = Trainer_ep(
            save_path=self.load_model, **self.config
        )
        self.model = NNModel_ep(self.data, self.trainer, **self.config) #
        ...
        #return y_pred #应该可以不加？因为加在预测的主程序里了

        #
        #print("self.data的类型：", type(self.data))
        #print(f"self.data的长度：{len(self.data)}")
        df = self.data[0]['raw_data'].copy()  ###
